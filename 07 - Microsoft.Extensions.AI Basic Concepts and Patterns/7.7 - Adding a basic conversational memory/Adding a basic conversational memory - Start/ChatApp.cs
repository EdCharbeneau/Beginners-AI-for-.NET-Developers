    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken) 
    {
        ...
        while (stoppingToken.IsCancellationRequested == false)
        {
            ...
            if (userMessage == null || exitRequested)
                break;
            ChatResponse chatResponse = await ai.GetResponseAsync(new ChatMessage(ChatRole.User, userMessage));
            Console.WriteLine($"{chatResponse.Messages[0].Role}: {chatResponse.Messages[0].Text}");
        }
    }
}